...
contract EvChargingMarket { 
    ...
    event PaymentSuccess(uint _aucId, address buyer, address seller, uint sechighprice, uint seclowprice, uint _energy); // event for successful payment
    event PaymentFailure(uint _aucId, address buyer, address seller, uint sechighprice, uint seclowprice, uint _energy);
    ...
    function revealReq(uint _aucId, uint _price, uint _amount, uint _reqId) public
    	auctionExist(_aucId)
        auctionClosed(_aucId) 
        revealNotEnded(_aucId)
    {
        ...
        if(contracts[_aucId].highestPrice != 0){
            if(_price > contracts[_aucId].highestPrice){
                contracts[_aucId].secondHighestPrice = contracts[_aucId].highestPrice;
                contracts[_aucId].highestPrice = _price;
                contracts[_aucId].amount = _amount;
                contracts[_aucId].buyer = msg.sender;
            } else {
                if(_price > contracts[_aucId].secondHighestPrice){
                    contracts[_aucId].secondHighestPrice = _price;
                }
            }
        } else {
            contracts[_aucId].highestPrice = _price;
    ...
    function updateBalance(uint _aucId) public 
        reportsOk(_aucId)   
    {
        if(contracts[_aucId].secondHighestPrice >= contracts[_aucId].secondLowestPrice){
            uint amount = contracts[_aucId].amount;                                       // amount is amount of electricity in kwh
            address buyer = contracts[_aucId].buyer;
            address seller = contracts[_aucId].seller;
            uint amountToPay = amount * contracts[_aucId].secondHighestPrice;                        // amountToPay is money owed = electricity in kwh * rate (second highest price)
            accounts[buyer].payments.push(Payment(_aucId, amount, true, amountToPay));       // payment added to buyer ledger (true is for bool toPay)
            accounts[buyer].balance -= int256(amountToPay);                                        // payment amount subtracted from balance
            accounts[buyer].energy += int256(amount);
            accounts[seller].payments.push(Payment(_aucId, amount, false, amountToPay));     // payment added to seller ledger (false is for bool toPay - is receiving)
            accounts[seller].balance += int256(amountToPay);                                       // payment added to seller balance   
            accounts[seller].energy -= int256(amount);
            emit PaymentSuccess(_aucId, buyer, seller, contracts[_aucId].secondHighestPrice, contracts[_aucId].secondLowestPrice, amount);
        }
        else{
            emit PaymentFailure(_aucId,
            			contracts[_aucId].buyer,
            			contracts[_aucId].seller,
            			contracts[_aucId].secondHighestPrice,
            			contracts[_aucId].secondLowestPrice,
            			contracts[_aucId].amount);
        }
    ...
    function storeAndLogNewReq(address _utility, uint _id, uint256 _time, uint256 _auctionTime) private {  // New request initialization from EV
        ...
        contracts[_id].highestPrice = 0;
        contracts[_id].secondHighestPrice = 0;
