...
# Create a nonce dictionary mapping address to nonce value
nonce_dict = {}
for buyer in ACCOUNTS_LIST[1:4]:
    tr = {
    'from': buyer,
    'nonce': Web3.toHex(w3.eth.get_transaction_count(buyer)),
    'gasPrice': gas_price,
    }
    nonce_dict[buyer] = [w3.eth.get_transaction_count(buyer), tr]
for address in nonce_dict:
    print(f"Nonce for {address[-4:]}: {nonce_dict[address][0]}")
count = 0
...
def send_ev_request(buyer_address):
    global nonce_dict
    tr = nonce_dict[buyer_address][1]
...
def ev_helper(buyer, auc_id, _time):
    global nonce_dict
    tr = nonce_dict[buyer][1]
    # Close the Auction
    nonce_dict[buyer][0] += 1
    tr['nonce'] = Web3.toHex(nonce_dict[buyer][0])
    close_auction(
        buyer,
        auc_id,
        _time,
...
async def log_loop(event_filter, poll_interval):
    ...
    # Make a threads list
    threads_list = []
    # Send a requests by an EV
    for address in ACCOUNTS_LIST[1:4]:
    ...
    # Making a set for buyers so no duplicate request happens
    buyer_set = set()

    while True:
        for LogReqCreated in event_filter.get_new_entries():
            if LogReqCreated['args']['buyer'] not in buyer_set:
                buyer_set.add(LogReqCreated['args']['buyer'])
                count += 1
                # send info to ev_helper()
                thread = threading.Thread(target = ev_helper,
                                          args = [LogReqCreated['args']['buyer'],
